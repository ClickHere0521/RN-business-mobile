...
const Scan = props => {
  ...
  const onSuccess = e => {
    const check = e.data;
    console.log('-------------------------------',e.data);
    const regex = /[a-zA-Z0-9]+[\/]+[a-zA-Z]+[_]+[a-zA-Z]+[\/]/g;
    setState({
      result: e,
      scan: true,
      ScanResult: true,
    ...
    if (regex.test(check)) {
      dispatch({type:actions.SET_COUPON_TICKET_PATH, payload:e.data})
  ...
  const showCoupon = () => {
      dispatch({type:actions.SET_COUPON_TICKET_PATH, payload:state.result.data})
    props.navigation.replace('CouponDetail');
  };
          ...
          <Fragment>
            ...
            {trueResult ? (
              <View style={ScanResult ? styles.scanCardView : styles.cardView}>
                ...
                <QRCode value={result.data} size={wp(65)} logo={QRLogoImg}/>
                <TouchableOpacity
                  onPress={showCoupon}
                  style={styles.buttonTouchable}>
                  <Text style={styles.buttonTextStyle}>Show Coupon</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View style={ScanResult ? styles.scanCardView : styles.cardView}>
